...
class EpsilonSpatioTemporalLSTMCell(nn.Module):
    def __init__(self, in_channel, num_hidden, width, filter_size, stride, layer_norm, epsilon=0.1):
        super(EpsilonSpatioTemporalLSTMCell, self).__init__()
    ...
    def epsilon_greedy(self, gate_value):
        """Apply epsilon-greedy exploration to the gate value."""
        if random.uniform(0, 1) < self.epsilon:
            return torch.rand_like(gate_value)  # Random exploration
        else:
            return gate_value  # Exploitation
    ...
    def forward(self, x_t, h_t, c_t, m_t):
        ...
        i_t = self.epsilon_greedy(torch.sigmoid(i_x + i_h))
        f_t = self.epsilon_greedy(torch.sigmoid(f_x + f_h + self._forget_bias))
        g_t = torch.tanh(g_x + g_h)

        c_new = f_t * c_t + i_t * g_t

        i_t_prime = self.epsilon_greedy(torch.sigmoid(i_x_prime + i_m))
        f_t_prime = self.epsilon_greedy(torch.sigmoid(f_x_prime + f_m + self._forget_bias))
        g_t_prime = torch.tanh(g_x_prime + g_m)

        m_new = f_t_prime * m_t + i_t_prime * g_t_prime

        mem = torch.cat((c_new, m_new), 1)
        o_t = self.epsilon_greedy(torch.sigmoid(o_x + o_h + self.conv_o(mem)))
        h_new = o_t * torch.tanh(self.conv_last(mem))
