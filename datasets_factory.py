from core.data_provider import kth_action, mnist, bair, asl
...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
    ...
    if  dataset_name == 'asl':
        input_param = {'paths': valid_data_list,
                       ...
                       'seq_length': seq_length,
                       'input_data_type': 'float32',
                       'is_output_sequence': True,  # Set based on your use case
                       'name': dataset_name + ' iterator'}
        input_handle = datasets_map[dataset_name].ASLInputHandle(input_param)
        input_handle.begin(do_shuffle=is_training)

        if is_training:
            train_input_handle = datasets_map[dataset_name].ASLInputHandle({
            ...
            train_input_handle.begin(do_shuffle=True)
            return train_input_handle, input_handle
        else:
            return input_handle
