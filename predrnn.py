...
import torch
import torch.nn as nn
from core.layers.SpatioTemporalLSTMCell import EpsilonSpatioTemporalLSTMCell  # Import the modified class
...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        for i in range(num_layers):
            in_channel = self.frame_channel if i == 0 else num_hidden[i - 1]
            cell_list.append(
                EpsilonSpatioTemporalLSTMCell(in_channel, num_hidden[i], width, configs.filter_size,
                                              configs.stride, configs.layer_norm, epsilon=0.1)  # Epsilon version
            )
        self.cell_list = nn.ModuleList(cell_list)
        self.conv_last = nn.Conv2d(num_hidden[num_layers - 1], self.frame_channel,
                                   kernel_size=1, stride=1, padding=0, bias=False)
